...
def test_register_user():
    ...
    response = tenancy_instance.user.create(f'upvest_test_{unique_id}','secret')
...
def test_list_user():
    ...
    user = create_user()
    response = tenancy_instance.user.retrieve(user['username'])
    assert response.status_code == 200
    assert response.data['username'] == user['username']

def test_list_users():
    """Tests an API call to get a list of usersr"""
    response = tenancy_instance.user.list()
    assert response.status_code == 200

def test_change_password():
    """Tests an API call to update a user's password"""
    user = create_user()
    response = tenancy_instance.user.update(user['username'], user['password'], 'new_secret')
    assert response.status_code == 200
    response = tenancy_instance.user.update(user['username'], 'new_secret', 'secret')
    assert response.status_code == 200

def test_deregister_user():
    """Tests an API call to deregister a user"""
    user = create_user()
    response = tenancy_instance.user.delete(user['username'])
